      ...
      >
        <div className="h-full overflow-y-auto">
          ...
          <div className="p-4 border-b-2 border-gray-900 bg-gray-50 sticky top-0 z-10">
            ...
            <p className="text-xs font-mono text-gray-600 uppercase">{mockBounties.length} Total</p>
          </div>
          ...
          <div className="p-2">
            {mockBounties.map((b) => {
              ...
              const isActive = currentBountyId === (b.id === -1 ? "example" : b.id.toString());

              return (
                <button
                  key={b.id}
                  onClick={() => {
                    router.push(`/bounties/${b.id === -1 ? "example" : b.id}`);
                ...
                >
                  <div className="flex items-start gap-2">
                    <div
                      className={`w-8 h-8 flex-shrink-0 border-2 border-gray-900 flex items-center justify-center ${
                        b.status === 1
                          ? "bg-green-500"
                          : b.status === 0
                          ? "bg-blue-500"
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-xs font-black uppercase tracking-tight leading-tight mb-1 truncate">
                        {bMetadata?.title || `Bounty #${b.id}`}
                      </h3>
                      <div className="flex items-center gap-2 text-[10px] font-mono text-gray-600">
                        <span className="font-bold">{formatETH(b.amount)} MNT</span>
                        <span className="text-gray-400">•</span>
                        <span>{b.submissions.length} subs</span>
                      </div>
                    </div>
                  </div>
                </button>
