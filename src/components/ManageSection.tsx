  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">Quest Management</h3>
        <div className="bg-blue-50 px-4 py-2 rounded-lg">
          <p className="text-sm text-blue-800">
            Select quests to manage entries, verify submissions, and handle administrative tasks.
          </p>
        </div>
      </div>
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          {[
            ...
            { key: 'finalize', label: 'Finalize Quests', icon: '🏁' },
            { key: 'cancel', label: 'Cancel Quests', icon: '✕' }
          ].map((tab) => (
            <button
              key={tab.key}
              onClick={() => setSelectedManageTab(tab.key as any)}
              className={`py-2 px-1 border-b-2 font-medium text-sm flex items-center gap-2 ${selectedManageTab === tab.key
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h4 className="text-lg font-semibold mb-4">My Quests</h4>

        {airdrops.filter((a) => a.creator === address).length === 0 ? (
          <div className="text-center py-8">
            <div className="text-gray-400 text-4xl mb-4">📢</div>
            <p className="text-gray-600 mb-2">You haven't created any quests yet.</p>
            <button
          ...
          <div className="grid gap-4">
            {airdrops
              ...
              .map((airdrop) => (
                <div
                  key={airdrop.id}
                  className={`border rounded-lg p-4 cursor-pointer transition-all hover:shadow-md ${selectedAirdropForManage?.id === airdrop.id
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div
                          className={`px-2 py-1 rounded-full text-xs font-medium ${airdrop.resolved
      ...
      {selectedAirdropForManage && (
        <div className="bg-white rounded-lg shadow p-6">
          {selectedManageTab === 'verify' && (
            <div>
              <h4 className="text-lg font-semibold mb-4 flex items-center gap-2">
                ✓ Verify Entries for "{selectedAirdropForManage.title || `Quest #${selectedAirdropForManage.id}`}"
              </h4>
              ...
              <div className="bg-blue-50 rounded-lg p-4 mb-6">
                ...
                <ol className="text-sm text-blue-800 space-y-1">
                  ...
                  <li>2. Check if it meets the quest requirements</li>
          ...
          {selectedManageTab === 'finalize' && (
            <div>
              <h4 className="text-lg font-semibold mb-4 flex items-center gap-2">
                🏁 Finalize "{selectedAirdropForManage.title || `Quest #${selectedAirdropForManage.id}`}"
              </h4>

              <div className="bg-yellow-50 rounded-lg p-4 mb-6">
                <h5 className="font-medium text-yellow-900 mb-2">⚠️ About finalizing quests:</h5>
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div>
                  <h6 className="font-medium mb-2">Quest Status</h6>
                  <div className="space-y-2 text-sm">
                    ...
                    <div>Status: <span className={`font-medium ${selectedAirdropForManage.resolved ? 'text-green-600' :
              ...
              >
                {selectedAirdropForManage.resolved ? 'Already Finalized' :
                  selectedAirdropForManage.cancelled ? 'Quest Cancelled' : 'Finalize Quest'}
              </button>
            </div>
          )}
          ...
          {selectedManageTab === 'cancel' && (
            <div>
              <h4 className="text-lg font-semibold mb-4 flex items-center gap-2">
                ✕ Cancel "{selectedAirdropForManage.title || `Quest #${selectedAirdropForManage.id}`}"
              </h4>

              <div className="bg-red-50 rounded-lg p-4 mb-6">
                <h5 className="font-medium text-red-900 mb-2">⚠️ About cancelling quests:</h5>
                <ul className="text-sm text-red-800 space-y-1">
                  <li>• You can only cancel quests with no approved entries</li>
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <h6 className="font-medium mb-2">Quest Details</h6>
              ...
              >
                {selectedAirdropForManage.cancelled ? 'Already Cancelled' :
                  selectedAirdropForManage.resolved ? 'Quest Completed' :
      ...
      {!selectedAirdropForManage && airdrops.filter((a) => a.creator === address).length > 0 && (
        <div className="text-center py-8 bg-gray-50 rounded-lg">
          ...
          <p className="text-gray-600">Select a quest above to manage its entries and settings</p>
        </div>
      )}
    </div>
  );
