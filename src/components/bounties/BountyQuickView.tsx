      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto rounded-[2rem] border border-white/60 bg-white/95 backdrop-blur-2xl shadow-2xl p-0">
        <div className="relative h-64 w-full bg-slate-100">
          ...
          <div className="absolute bottom-6 left-8 right-8">
            <h2 className="text-2xl font-bold text-white mb-1">{metadata?.title || "Bounty Details"}</h2>
        ...
        <div className="p-8 space-y-8">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100">
              ...
              <p className="text-lg font-black text-slate-700">{bounty.submissions.length}</p>
            </div>
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100">
              <p className="text-[10px] uppercase tracking-wider font-bold text-slate-400 mb-1">Time Left</p>
              <p className="text-sm font-bold text-slate-700">{formatTimeLeft(bounty.deadline)}</p>
            </div>
          </div>
