...
const NftBadge: React.FC<NftBadgeProps> = ({ level, badgeType }) => {
  ...
  return (
    <div className={`w-48 h-64 border-4 ${colors.border} ${colors.bg} p-4 flex flex-col items-center justify-between shadow-lg ${colors.shadow} transform transition-transform`}>
      <div className={`font-bold text-lg ${colors.text}`}>{badgeLevel}</div>
      <div className="w-24 h-24 bg-white flex items-center justify-center border-4 border-white shadow-inner">
